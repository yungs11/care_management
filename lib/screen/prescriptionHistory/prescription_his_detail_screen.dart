...
class PrescriptionHistoryDetailScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PrescriptionHistoryDetailScreen> createState() =>
      _PrescriptionHistoryScreenState();
}

class _PrescriptionHistoryScreenState
    extends ConsumerState<PrescriptionHistoryDetailScreen> {
  ...
  Future<List<TimezoneBoxModel>> fetchPrescriptionData() async {
    ...
    try {
      final planList = await umService.fetchCardList(
        widget.selectedDate,
      );

      timezoneList = planList
...
DataRow renderPillItem(MedicineItemModel pill) {
  ...
  return DataRow(
    cells: <DataCell>[
      ...
      DataCell(Container(
          width: 40.0, child: Text('${pill.takeAmount.toString()}개'))),
      DataCell(Container(
          width: 40.0, child: Text('${pill.remainAmount.toString()}일분'))),
      DataCell(Container(
          width: 40.0, child: Text('${pill.totalAmount.toString()}일분'))),
    ],
  );
}
