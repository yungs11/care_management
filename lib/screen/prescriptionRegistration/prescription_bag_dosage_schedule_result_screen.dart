...
import 'package:care_management/common/model/prescription_schedule_box_model.dart';
...
class _PrescriptionBagDosageSceduleResultScreenState
  ...
  List<MedicationItem> copiedMedicine = [];
  List<MedicationItem> copiedToMedicine = [];
  Map<int, PrescriptionScheduleBoxModel> medicinesPerBox = {
    1: PrescriptionScheduleBoxModel(title: '아침 (08:00)', medicines: [
      MedicationItem(name: '코푸시럽', selected: true),
      MedicationItem(name: '잘티진정', selected: false),
      MedicationItem(name: '잘티콜록정', selected: false),
      MedicationItem(name: '잘티콜록정', selected: false),
      MedicationItem(name: '잘티콜록정', selected: false),
    ]),
  ...
  Widget renderMedicineBox(int boxId,  String title, List<MedicationItem> medicines) {
      return InkWell(
        onTap: () {
          setState(() {
